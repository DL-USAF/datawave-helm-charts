...
from helpers.query import QueryParams, QueryConnection
from utilities import setup_logger, ascii_fail, assert_test, log_test_start
from helpers.subtest import Test

cert = 'resources/mock_server.pem'
ip = 'localhost'
port = '8443'


def test_query(log: Logger):
...
def test_query_fields(log: Logger):
    ...
    for field in fields:
        ...
        with QueryConnection(ip, port, cert, query_params, log=log) as qc:
            ...
            for data in qc:
                log.debug(f"Number of Returned Events: {data['ReturnedEvents']}")
                event_count += data['ReturnedEvents']
            if not event_count:
...
def test_query_data(log: Logger):
    ...
    with QueryConnection(ip, port, cert, query_params, log=log) as qc:
        event_count = 0
        for queried_data in qc:
            log.debug(f"Number of Returned Events: {queried_data['ReturnedEvents']}")
            event_count += queried_data.get('ReturnedEvents')
...
def test_auth(log: Logger):
    ...
    with test.subtest("Test PRIVATE auth"):
        ...
        with QueryConnection(ip, port, cert, query_params, log=log) as qc:
            for queried_data in qc:
                for event in queried_data['Events']:
                    name = next(field['Value']['value'] for field in event['Fields'] if field['name'] == 'NAME')
                    actual.add(name)
        if actual != expected:
